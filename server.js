...
app.get("/api/protected", authenticateToken, (req, res) =>{    //en skyddad route som bara kan nås om användaren har giltig jwt token
    const formateradDate = new Date(req.user.date).toISOString().split("T")[0]  //Formaterar om registreringsdatumen till en iso sträng och tar bara datumdelen
    res.json({message: "Skyddad route", user: {username: req.user.username, date: formateradDate}})
})
...
function authenticateToken(req, res, next){        //middleware som kontroller ifall en giltig jwt skickas med requesten
    ...
    jwt.verify(token, process.env.JWT_SECRET_KEY, (err, decoded) => {         //verifierar token med hjälp av hemliga nyckeln om den är giltig skickas payloaden tillbaka annars uppstår error
